...
<template>
  <div
    class="shrink-0 grow-1 bg-[#faf9fa] px-10 py-5 flex flex-col justify-start gap-10"
  >
    <h1 class="font-bold text-xl">Connection details</h1>

    <form
      class="grid grid-cols-3 gap-y-3 py-2 pr-5 align-middle items-start overflow-y-scroll"
    >
      ...
      <textarea
        class="col-start-2 col-span-2 border-2 border-teal-500 rounded-lg px-2 py-1 self-start overflow-y-auto resize-y min-h-16 max-w-full"
      ...
      >
        ...
        <icon
          :name="
            showExtraOptionsDropdown ? 'ph:folder-open-bold' : 'ph:folder-bold'
          "
      ...
      <template v-if="showExtraOptionsDropdown">
        <label class="col-start-1 col-span-1 select-none font-bold"
          >Group:</label
        >

        <insertable-dropdown
          class="border-2 border-teal-500 rounded-lg col-start-2 col-span-2"
          :options="groups"
    ...
    <div class="grid grid-cols-2 gap-x-3 justify-between">
      <button
        ...
        @click="handleSave"
        class="text-center bg-indigo-900 text-white rounded-lg border-2 w-full py-1 hover:shadow-lg hover:cursor-pointer disabled:bg-transparent disabled:border-2 border-stone-400 disabled:text-stone-400 disabled:cursor-not-allowed disabled:shadow-none"
      >
        Save
      </button>
      <button
        @click="handleDelete"
        class="text-center border-red-500 text-red-500 rounded-lg border-2 w-full py-1 hover:shadow-lg hover:cursor-pointer"
      >
        Delete
      </button>
    </div>
