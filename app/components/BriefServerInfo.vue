...
<template>
  <div class="hover:bg-[#dcdcdc] hover:cursor-pointer rounded-lg px-2 py-1 flex flex-row items-center justify-between group"
  >
    <section
        class="grow-1 flex flex-row items-center gap-3"
        @click="emit('rowClicked')"
    >
      <p class="flex flex-col justify-center items-center font-bold text-white size-10 rounded-full bg-teal-600">
      ...
      <div>
        ...
        <p>
          <icon v-if="status === LandingScreenServerStatus.AVAILABLE" name="ph:plugs-connected-bold" class="text-lg align-middle text-green-600" />
          <icon v-else-if="status === LandingScreenServerStatus.UNAVAILABLE" name="ph:plugs-bold" class="text-lg align-middle text-red-600" />
          <icon v-else-if="status === LandingScreenServerStatus.PENDING" name="ph:circle-notch-bold" class="text-lg align-middle text-gray-600 animate-spin" />
          {{ server.address }}
        </p>
      </div>
    </section>
