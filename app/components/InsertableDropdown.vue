...
<template>
  <div class="relative">
    <div class="flex flex-row items-center px-2 py-1">
      ...
      <icon
          :name="isOpen ? 'ph:arrow-circle-down-bold' : 'ph:arrow-circle-left-bold'"
    ...
    <ol v-if="isOpen"
        class="
          border-2 border-teal-500 rounded-b-lg
          absolute z-10 bg-gray-100 w-full max-w-full
          space-y-1 p-1"
    >
      <li class="flex flex-row items-center space-x-2">
        <input type="text" placeholder="New group" class="grow-1" v-model="customGroup" />
        <icon
            name="ph:plus-circle-bold" class="align-middle px-2 py-1 text-2xl hover:cursor-pointer"
            @click="handleDropdown(customGroup)"
        />
      </li>
      <li
          v-for="(group, groupIndex) in options" :key="groupIndex"
